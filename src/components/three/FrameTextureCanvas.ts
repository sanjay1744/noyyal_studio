...
export function createFrameTexture(
  ...
  const galleryImages = project.gallery && project.gallery.length > 0
    ? project.gallery
    : [project.heroImage];
  const safeIdx = Math.abs(imageIndex) % galleryImages.length;
  const currentImageUrl = galleryImages[safeIdx] || project.heroImage;

  const img = getOrLoadImage(currentImageUrl, () => {
    if (onImageLoaded) onImageLoaded();
  });

  if (img && img.complete) {
    // Calculate aspect ratio fit (Contain / Cover inside inner canvas)
